...
## The Exploit
I tried `administrator'--` into the username field.Also inserted some random password in order to bypass the `Empty field` restriction.

## What I Learned
Most of the empty field SQL Queries are run by the `SELECT * FROM products WHERE USERNAME = <username> AND password = <passwd>`.So,by adjusting queries and commenting out unwanted parameters,we can access the accounts regardless of the password.
